...
export class ProductService {
    ...
    getProducts(filter: ProductFilter = undefined): Observable<Product[]> {
        ...
        // Check if filter has any parameter
        if (filter !== null) {
            if (Object.prototype.hasOwnProperty.call(filter, 'text') && !Object.prototype.hasOwnProperty.call(filter, 'category')) {
                // Has only a 'text' parameter
                search.set("q", filter.text);
            } 
            else {
                if (!Object.prototype.hasOwnProperty.call(filter, 'text') && Object.prototype.hasOwnProperty.call(filter, 'category')) {
                    // Has only a 'category' parameter
                    search.set("category.id", filter.category);
                }
                else {
                    // Has a 'text' and 'category' parameters
                    search.set("q", filter.text);
                    search.set("category.id", filter.category);
                }
            }
            if (Object.prototype.hasOwnProperty.call(filter, 'state')) {
